<template>
  <div class="d-flex justify-center my-10">
    <v-card width="95vw" rounded="xl">
      <div class="d-flex flex-row align-stretch">
        <v-card class="pa-10 rounded-0 rounded-l-xl" height="900" width="50%" dark>
          <v-card-title class="mb-5" primary-title>
            <h2>
              <v-icon left class="primary--text text-h3">mdi-arm-flex</v-icon>
              สร้างคอร์สออกกำลังกายของคุณ
            </h2>
          </v-card-title>
          <v-card-text class="text-subtitle-1"
            >4 อันดับคอร์สที่ประสบความสำเร็จแห่งปีใน Trainder</v-card-text
          >
          <v-card rounded="xl">
        ...
        <div style="width: 50%" class="pa-0 ma-0 ml-3">
          <v-form class="pa-5 mt-12" ref="Courseform" @submit.prevent="CourseCreate">
            <v-text-field
            ...
            <p
              ...
              v-if="CourseData.start >= CourseData.end"
            >
              **วันเริ่มคอร์สต้องน้อยกว่าวันจบคอร์ส
            </p>

            <v-text-field
              label="สร้างตารางออกกำลังกายที่นี่"
              outlined
              readonly
              prepend-inner-icon="mdi-calendar"
              :disabled="CourseData.start >= CourseData.end"
              @click="Table_dia = true"
              :value="events.length == 0 ? '' : 'กดที่นี่เพื่อแก้ไขตารางกิจกรรม'"
            ></v-text-field>
            <p align="left" style="font-size: 12px; color: black">
              **หากต้องการแก้ไขวันเริ่มและจบคอร์ส กรุณาล้างตารางเดิมก่อน
            </p>
    ...
    <v-dialog v-model="Table_dia" width="1000" height="1000"
      ><v-card>
            ...
            <td width="1400">
              <div class="diabox">
                <v-sheet tile height="54" class="d-flex">
                  ...
                  <v-btn v-if="type != 'day'" outlined class="ma-2" @click="events = []">
                    ล้างกิจกรรมทั้งหมด
                  </v-btn>
                ...
                <v-card-text align="left" color="teal accent-4"
                  >**กิจกรรมในคอร์สจะถูกเพิ่มอัตโนมัติในตารางงานของท่าน</v-card-text
                >
                <!-- $store.state.events -->
              </div>
            </td>
      ...
      <v-card>
        <v-container>
          <v-form ref="addEventform" @submit.prevent="addEvent">
            ...
            ></v-text-field>
            <p style="font-size: 15px; color: red" v-if="eventstart >= eventend">
            ...
            <p
              ...
              v-if="
                !(
                  CourseData.start <= this.eventstart &&
                  eventstart < this.CourseData.end &&
                  CourseData.start < this.eventend &&
                  eventend < this.CourseData.end
                )
              "
            >
              **กิจกรรมต้องอยู่ในช่วงเวลาของคอร์สเท่านั้น
            </p>
          </v-form>
        </v-container>
      </v-card>
    ...
    <v-dialog v-model="snackcollide" max-width="400">
      <v-card dark color="white">
        ...
        <v-card-text>
          <p style="color: gray">
            ไม่สามารถสร้างคอร์สได้ เนื่องจากมีกิจกรรมของท่านซ้อนทับกับกิจกรรมในคอร์ส
            กรุณาปรับเปลี่ยนกิจกรรมใหม่ แล้วลองอีกครั้ง
          </p>
        </v-card-text>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="snacksuccess" max-width="400">
      <v-card dark color="white">
        ...
        <v-card-text>
          <p style="color: gray">
            เพิ่มคอร์สใหม่เรียบร้อยแล้ว กรุณาตรวจสอบคอร์สของท่านที่หน้ารายละเอียดคอร์ส
          </p>
        </v-card-text>
      </v-card>
    </v-dialog>
  ...
  methods: {
    ...
    async addEvent() {
      if (
        ...
        this.CourseData.start <= this.eventstart &&
        this.eventstart < this.CourseData.end &&
        this.CourseData.start < this.eventend &&
        this.eventend < this.CourseData.end
      ) {
        //+"T00:00"
        ...
        var collision = await this.isCollision(
          this.eventstart,
          this.eventend,
          this.events
        );
    ...
    async isCollision(newEvent_start, newEvent_end, allevent) {
      ...
      for (const oldEvent of allEvent) {
        ...
        if (
          (newEvent_start >= oldEvent.start && newEvent_start <= oldEvent.end) ||
    ...
    async checkEventTable(allEvent) {
      ...
      for (let key in this.events) {
        ...
        eventcollide = await this.isCollision(
